...
function optionchange(val) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			html +=
				' <input type="hidden" name="ak_pre_optionid[]" value="' + val + '" />';
			html +=
				' <input type="hidden" name="product_option[' +
				option_row +
				'][type]" value="' +
			...
			if (
				...
				for (i = 0; i < item["option_value"].length; i++) {
					html +=
						'  <option value="' +
						item["option_value"][i]["id"] +
						'">' +
						item["option_value"][i]["optionKey"] +
						"</option>";
				}
			...
			$("#option > li:last-child").before(
				'<li><a href="#tab-option' +
					option_row +
					"\" data-toggle=\"tab\"><i class=\"fa fa-minus-circle\" onclick=\" $('#option a:first').tab('show');$('a[href=\\'#tab-option" +
					option_row +
					"\\']').parent().remove(); $('#tab-option" +
					option_row +
					"').remove();\"></i>" +
					item["label"] +
					"</li>"
...
function addOptionValue(option_row) {
	...
	html +=
		...
		option_value_row +
		'][option_value_id]" class="form-control">';
	...
	html +=
		...
		"][product_option_value][" +
		option_value_row +
		'][product_option_value_id]" value="" /></td>';

	html +=
		'  <td class="text-right"><input type="number" name="product_option[' +
		option_row +
		"][product_option_value][" +
		option_value_row +
	...
	html +=
		'<td class="text-left"><button type="button" onclick="$(\'#option-value-row' +option_value_row +
		'\').remove();"  class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += "</tr>";
